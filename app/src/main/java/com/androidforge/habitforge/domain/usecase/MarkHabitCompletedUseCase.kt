...
class MarkHabitCompletedUseCase @Inject constructor(private val repository: HabitRepository) {
    suspend operator fun invoke(habitId: Long): Result<Unit> {
        val today = DateUtils.today()
        return repository.markHabitCompletion(habitId, today, CompletionType.COMPLETED)
    }
}